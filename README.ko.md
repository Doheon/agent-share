...
- Podman / Docker 사용 가능 여부 확인 (`ash serve`에 필요)
...
TUI가 뜹니다. 프롬프트를 입력하면 네트워크가 처리할 피어를 찾고, diff가 표시되고, 적용/스킵을 결정할 수 있습니다. **다른 피어가 벌어둔 크레딧을 사용하는 셈입니다.**
...
CPU를 공유해도 되는 머신(가능하면 별도 머신)에서:

```bash
ash serve              # 무한정 수락
ash serve -n 5         # 5개까지 수락 후 종료
```

수락자는 요청자의 암호화된 코드를 받아 AI 에이전트를 Podman/Docker 샌드박스에서 실행하고, diff를 돌려보냅니다. **요청자가 diff를 적용하면 크레딧이 원자적으로 로컬 원장에 적립됩니다.**

### 4. 크레딧 획득 — ash 레포에 기여

```bash
ash mine               # 자동 사이클 1개 작업
ash mine -n 3          # 최대 3개 작업
ash mine "history command misses mint events"  # 코드 증거와 함께 이슈 등록
```

mine은 ash 자체에 기여한 대가로 크레딧을 지급합니다: 이슈 구현, PR 리뷰, 증거가 있는 버그 리포트 등.

### 5. 잔액 확인

```bash
ash status             # 사용자명 · 잔액 · pubkey · 에이전트 로그인 상태
ash history            # earn/spend/mint 이벤트 전체 로그
ash peers              # 온라인 피어와 잔액
```
...
| 목적 | 명령어 |
|------|--------|
| 현재 프로젝트에 AI 프롬프트 실행 | `ash` (TUI) 또는 `ash run "<프롬프트>"` |
| 작업 수락으로 크레딧 획득 | `ash serve` |
| ash 레포 개선으로 크레딧 획득 | `ash mine` |
| 검증된 증거로 버그 리포트 등록 | `ash mine "<쿼리>"` |
| 모델 변경 (haiku / sonnet / opus / codex) | `ash set <티어>` |
| 신원과 잔액 확인 | `ash status` |
| 이벤트 로그 보기 | `ash history` |
| 온라인 피어 목록 | `ash peers` |

TUI 안에서는 모든 명령이 슬래시 커맨드 (`/serve`, `/mine`, `/model`, `/status`, `/history`, `/peers`, `/login`, `/help`, `/quit`).
...
sequenceDiagram
    ...
    R->>DHT: task:announce (암호화된 blob)
    DHT-->>A: 작업 가능
    A->>R: task:claim (RSA 공개키, 원자적)
    R->>A: AES 키 (RSA-OAEP 래핑)
    Note over A: 코드 복호화 · AI 실행<br/>(Podman / Docker 샌드박스)
